function [ out ] = eightExpertWeightCommon( U,x,featureMatrixOfExpert )
%修正后的专家权重计算
...
% vfinal是群体一致偏好
for i=1:7
    vfinal(i)=vfinal(i)/(x(1)*featureMatrixOfExpert(i,1)+x(2)*featureMatrixOfExpert(i,2)+x(3)*featureMatrixOfExpert(i,3)+x(4)*featureMatrixOfExpert(i,4)+x(5)*featureMatrixOfExpert(i,5)+x(6)*featureMatrixOfExpert(i,6)+x(7)*featureMatrixOfExpert(i,7)+x(8)*featureMatrixOfExpert(i,8));
end
...
g=zeros(1,7);
for i=1:7
    g(i)=x(1)*featureMatrixOfExpert(i,1)+x(2)*featureMatrixOfExpert(i,2)+x(3)*featureMatrixOfExpert(i,3)+x(4)*featureMatrixOfExpert(i,4)+x(5)*featureMatrixOfExpert(i,5)+x(6)*featureMatrixOfExpert(i,6)+x(7)*featureMatrixOfExpert(i,7)+x(8)*featureMatrixOfExpert(i,8);
end
