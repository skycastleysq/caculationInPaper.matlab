% function [sum , paixuf]=four_Pros_SimilaryCommon(x1L,x1U,x2L,x2U,x3L,x3U,x4L,x4U)
 clc;clearvars;
format short;
lamda=2.25;
afa=0.88;
...
featureMatrixOfExpert=zeros(7,5);
schemeNoOfEachExpert=zeros(1,5);
...
for k=1:5
    for i=1:7
        for j=1:3
            if(XL(i,j,k)~=0)
                  vp_(i,j,k)=(getdis(RL(i,j),RU(i,j),RfnL(k,j),RfnL(k,j)))^afa;
                  vn_(i,j,k)=-lamda*(getdis(RL(i,j),RU(i,j),RfnL(k,j),RfnL(k,j)))^bta;
            end
...
% ub3=[0.42;0.45;0.43];
x3_0=[0.3,0.3,0.4];
[x3,fval3]=fmincon(@(x3)attributeWeightCommon(v3p,v3n,x1,3,featureMatrixOfExpert,schemeNoOfEachExpert),x3_0,[],[],Aeq3,beq3,lb3,ub3);
disp('最优解x3');disp(x3);
...
Aeq=[1,1,1,1,1];
beq=1;
lb=[0.20;0.20;0.20;0.20;0.20];
ub=[0.30;0.30;0.30;0.30;0.30];
x0=[0.25,0.25,0.25,0.25];


[rou,fval]=fmincon(@(rou)fourExpertWeightCommon(U,rou),x0,[],[],Aeq,beq,lb,ub);
...
sum(5)=sum(5)/(rou(1)+rou(2)+rou(4)+rou(5));
